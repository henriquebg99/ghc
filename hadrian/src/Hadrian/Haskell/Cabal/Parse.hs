...
import Settings.Builders.Common (packageDatabaseArgs, topDirectory)
...
configurePackage context@Context {..} = do
    ...
    case C.buildType (C.flattenPackageDescription gpd) of
      C.Simple -> traced "cabal-configure" $ C.defaultMainWithHooksNoReadArgs C.simpleUserHooks gpd argList'
      ...
      C.Custom -> do
        ...
        cmd_ ghc $ concat
          ...
          , [pkgPath package -/- "Setup.hs", "-outputdir", dir, "-o", dir -/- "Setup" <.> exe]
          ]
        hadrianEnv <- if
          | package == ghcBoot -> do
          ...
          | package == compiler -> do
            ...
            pure $ show configEnv
          | otherwise -> pure "[]"
        cmd_ (AddEnv "HADRIAN_SETTINGS" hadrianEnv) (dir -/- "Setup" <.> exe) argList'
