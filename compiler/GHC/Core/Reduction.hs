...
When we rewrite ty at role r, producing Reduction ty' dco xi, we guarantee:
  ...
  RW2: dco :: ty' ~r xi
  RW3: followDCo r ty' dco is equal to xi (up to zonking)

We need to use ty', and not ty, to satisfy RW3; see Note [The Hydration invariant]
in GHC.Core.Coercion. This is why we store the LHS type in the Reduction too.
