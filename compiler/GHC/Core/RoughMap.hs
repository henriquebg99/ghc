...
e.g., We may have an environment which includes

> data instance Fam Int a = ...

which will result in

> axiom ax :: Fam Int ~ FamInt

and an FamInst with

> fi_tcs = [Int]`, `fi_eta_tvs = [a]

But we need to make sure that this instance matches `Fam Int a` when it's inserted
into the RoughMap with key `[RM_KnownTC Fam, RM_KnownTC Int]`. To achieve this `insertRM` will
add the axiom to the `rm_empty` field when the key is exhausted so the trie has structure like this:

rm_known = Fam { rm_known = Int { rm_empty = Inst }}

Then when the lookup happens with key  [RML_KnownTC Fam, RML_KnownTC Int, RML_WildCard]
the relevant clause of `lookupRM` will return `rm_empty` when looking for `RML_WildCard`.
...
Where mightMatch is defined like this:
  ...
  mightMatch []  (_:_) = False  -- If the lookup key is longer, no match
  ...
  mightUnify []  (_:_) = False  -- If the lookup key is longer, no unification
