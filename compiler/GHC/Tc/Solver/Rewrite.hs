...
rewrite_args_tc tc roles args =
  do { traceRewriteM "rewrite_args_tc {" (text "tc:" <+> ppr tc)
     ; res <- rewrite_args all_bndrs any_named_bndrs inner_ki emptyVarSet roles args
     ; traceRewriteM "rewrite_args_tc }" $
         vcat [ text "tc:" <+> ppr tc
              , text "res:" <+> ppr res ]
     ; return res }
...
-- Note [No derived kind equalities]
  = do { traceRewriteM "rewrite_args_slow {" $
           vcat [ text "tys:" <+> ppr tys ]
       ; rewritten_args <- zipWith3M rw (map isNamedBinder binders ++ repeat True)
                                        roles tys
       ...
       ; let res = simplifyArgsWorker binders inner_ki fvs roles rewritten_args
       ; traceRewriteM "rewrite_args_slow }" $
           vcat [ text "tys:" <+> ppr tys
                , text "rewritten_args:" <+> ppr rewritten_args
                , text "res:" <+> ppr res ]
       ; return res }
  where
    {-# INLINE rw #-}
  ...
  = do { het_redn <- case tcSplitTyConApp_maybe fun_xi of
           Just (tc, xis) ->
             do { let tc_roles  = tyConRolesRepresentational tc
                ...
                ; return $
                   mkHetReduction (mkReduction app_ty app_co app_xi) kind_co }
           Nothing ->
...
rewrite_exact_fam_app tc tys
  = do { checkStackDepth tyconapp_ty
    ...
    do { eq_rel <- getEqRel
       ...
       ; let
           args_co = mkTyConAppRedn tc redns
