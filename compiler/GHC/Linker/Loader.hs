...
{- 
Note [Loader Consistency Checks]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
GHC can't in general load code generated by a different build of GHC, even if the two
GHC's are the exact same version. This is due to subtle differences in ABIs that can
manifest as mysterious segfaults when running loaded TH or plugin code.

This can be a major problem when distributing binaries that statically link
against the GHC API, and also when "reinstalling" the GHC library. The GHC
linked by the binary may be a different ABI version than the one available on
the users machine, and similarly the "reinstalled" GHC may have a different
ABI hash than the original GHC.

To avoid such situations, we use the ghc-abihash package to embed in the ABI
hash for the current GHC build and all its (transitive dependencies), and refuse
to load any package with the same UnitId but a different ABI hash.
See #20742 for more discussion of this problem.

Users can augment this list of hashes with their own values if they need to
define their own plugin system for their application over and above GHC's plugin
system.
-}
...
loadPackage _      hsc_env pkg
  ...
  , let msg = text "Attempting to load dependency" <+> text (unitPackageNameString pkg) -- <+> text "due to" <+> doc
           ...
           $$ text "But GHC is linked against a different build of this dependency, with ABI hash" <+> quotes (text (ST.unpack linked_abi_hash))
           $$ text "Rejecting this as loading code with ABIs different from the current GHC process is not supported"
  = throwGhcExceptionIO (ProgramError $ showSDoc dflags msg)
  where
    dflags = hsc_dflags hsc_env
